...
describe("Trips", () => {
  ...
  it("should store all destinationData", function () {
    expect(userTrips.destinationData).to.deep.equal({
      destinations: [
        {
          id: 6,
          destination: "Jakarta, Indonesia",
  ...
  it("should be able to calculate the cost of one trip", function () {
    ...
    expect(userTrips.calculateSingleTripCost(6, 10, 6)).to.equal("$6,644.00");
    expect(userTrips.calculateSingleTripCost(6, 10, 51)).to.equal(
      "Invalid destination"
    );
  });
});
